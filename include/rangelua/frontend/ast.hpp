...
class ASTVisitor;
template<typename T> class ASTVisitorT;
...
public:
    ...
    explicit LiteralExpression(Value value, SourceLocation location = {}) noexcept
        : Expression(NodeType::Literal, std::move(location))
...
public:
    explicit IdentifierExpression(String name, SourceLocation location = {}) noexcept
        : Expression(NodeType::Identifier, std::move(location))
        , name_(std::move(name)) {}
...
public:
    ...
    BinaryOpExpression(Operator op, ExpressionPtr left, ExpressionPtr right, SourceLocation location = {}) noexcept
...
public:
    FunctionCallExpression(ExpressionPtr function, ExpressionList arguments, SourceLocation location = {}) noexcept
...
public:
    explicit BlockStatement(StatementList statements, SourceLocation location = {}) noexcept
        : Statement(NodeType::Block, std::move(location))
...
public:
    AssignmentStatement(ExpressionList targets, ExpressionList values, SourceLocation location = {}) noexcept
...
public:
    ...
    IfStatement(ExpressionPtr condition, StatementPtr then_body,
    ...
    [[nodiscard]] const std::vector<ElseIfClause>& elseif_clauses() const noexcept { return elseif_clauses_; }
...
public:
    explicit Program(StatementList statements, SourceLocation location = {}) noexcept
        : ASTNode(NodeType::Program, std::move(location))
        , statements_(std::move(statements)) {}
