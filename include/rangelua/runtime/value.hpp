...
public:
    ...
    [[nodiscard]] bool is_boolean() const noexcept { return std::holds_alternative<Boolean>(data_); }
    [[nodiscard]] bool is_number() const noexcept { return std::holds_alternative<Number>(data_); }
    [[nodiscard]] bool is_string() const noexcept { return std::holds_alternative<String>(data_); }
    [[nodiscard]] bool is_table() const noexcept { return std::holds_alternative<TablePtr>(data_); }
    [[nodiscard]] bool is_function() const noexcept { return std::holds_alternative<FunctionPtr>(data_); }
    [[nodiscard]] bool is_userdata() const noexcept { return std::holds_alternative<UserdataPtr>(data_); }
    [[nodiscard]] bool is_thread() const noexcept { return std::holds_alternative<ThreadPtr>(data_); }
...
inline Value nil() noexcept { return Value{}; }
inline Value boolean(bool b) noexcept { return Value(b); }
inline Value number(Number n) noexcept { return Value(n); }
inline Value integer(Int i) noexcept { return Value(i); }
inline Value string(String s) { return Value(std::move(s)); }
inline Value string(StringView s) { return Value(s); }
inline Value string(const char* s) { return Value(s); }
 ...
 */
template<typename T>
concept ConvertibleToValue = requires(T t) {
    Value(t);
};
...
// Concept verification - temporarily disabled until forward declaration issues are resolved
